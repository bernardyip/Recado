...
<body>
	...
	<p>$$$$EZPZ MONEY$$$$ SIGN UP NOW ABOVE AND REAPS THE BENEFIT OF YOUR
		LIFETIME</p>
	<p>Here are some tasks for you to see:</p>
		...
		while ( $row = pg_fetch_array ( $result ) ) {
			...
			$count_result = pg_execute ( $dbcon, 'count_category_query', array (
					$row ['id'] 
			) );
	...
	<?php
			...
			pg_prepare ( $dbcon, 'select_task_in_category_query', "SELECT t.id, t.name, t.description FROM public.task t WHERE t.category_id=$1 LIMIT 3" );
			$cat_result = pg_execute ( $dbcon, 'select_task_in_category_query', array (
					$row ['id'] 
			) );
